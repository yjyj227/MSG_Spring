...
@Component
@Controller
public class G_WriteController {
	...
	//1. 글쓰기폼으로 이동
	@RequestMapping(value="/G_Write.do", method=RequestMethod.GET)
	public String form() {
		System.out.println("초기화 form() 호출됨");
		return "/board/ghibli/G_Write";
	}
	...
	//3. 입력해서 유효성검사->에러 발생
	@RequestMapping(value="/G_Write.do", method=RequestMethod.POST)
	public String submit(@ModelAttribute("command") GBoardDTO com,
		...
		if (log.isDebugEnabled()) {
			System.out.println("/G_Write.do 요청중");
		...
		//에러정보가 있다면
		if (result.hasErrors()) {
			return form(); //"G_Write" -> "G_Write.jsp"로 이동해라
		}
		*/
		...
		try {
			...
			if (!com.getUpload().isEmpty()) {
				newName=G_FileUtil.rename(com.getUpload().getOriginalFilename());
				System.out.println("newName=>"+newName);
				com.setG_filename(newName);
			}
			//-----파일 업로드-----
			//최댓값+1
			newNum=gBoardDao.getNewNum()+1;
			System.out.println("newNum=>"+newNum);
			//게시물번호 저장
			com.setG_number(newNum);
			//글쓰기 호출
			int insert=gBoardDao.insert(com);
			if (!com.getUpload().isEmpty()) {
				File file=new File(G_FileUtil.UPLOAD_PATH+"/"+newName);
				com.getUpload().transferTo(file);
			}
			System.out.println("글 작성 성공 여부(insert)=>"+insert);
			if (insert > 0) {
				String mem_id=com.getMem_id();
				System.out.println("G_WriteController의 mem_id=>"+mem_id);
				int pointup=gBoardDao.pointupA(mem_id);
		...
		return "redirect:/G_Content.do?g_number="+newNum;
	}
