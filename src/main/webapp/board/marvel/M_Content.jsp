			...
			<div class="row">
				<div class="col-md-offset-1 col-md-10 col-md-offset-1">
					...
					<table class="table">
							...
							<tr align="center">
								...
								<c:choose>
									...
									<c:when test="${article.m_category==34}">
										<font style="color: gray">[블랙 팬서]&nbsp;&nbsp;&nbsp;</font>
									</c:when>
								...
								<c:choose>
									<c:when test="${article.m_ref==1}">
										<font style="color: gray;">[자유]&nbsp;</font>
									</c:when>
									<c:when test="${article.m_ref==2}">
						...
						<tr>
							<td>
								<div class="col-md-1">${article.m_nickname}</div>
								<div class="col-md-7"></div>
								<div class="col-md-2">
									<fmt:formatDate value="${article.m_date}" timeStyle="medium"
										pattern="yy.MM.dd  HH:mm" />
								</div>
								<div class="col-md-1">조회 ${article.m_count}</div>
								<div class="col-md-1">스크랩 ${article.m_scrap}</div>
							</td>
						</tr>
						...
						<tr>
							<td>
								...
								<c:if test="${!empty article.m_filename}">
								<img src="./board/marvel/upload/${article.m_filename}" width="60%">
								<br><br>
								</c:if>
								${fn:replace(article.m_body, replaceChar, "<br/>")}
								</div>
							</td>
						</tr>
						<tr>
								...
								<div class="col-md-12">
									<div class="col-md-2" align="left">
										<button type="button" class="btn btn-default btn-sm"
											onclick="document.location='M_List.do?m_ref=${article.m_ref}'">목록으로</button>
									</div>
									...
									<div class="col-md-8" align="right">
										<c:if test="${!empty mem_id}">
											<c:if test="${article.mem_id!=mem_id}">
												...
												onclick="document.location='Scrap.do?s_number=${article.m_number}&s_category=${article.m_category}&s_title=${article.m_title}&s_nickname=${article.m_nickname}'">스크랩</button>
											</c:if>

											<c:if
												test="${article.mem_id==mem_id || mem_id.equals('admin')}">
												<button type="button" class="btn btn-warning btn-sm"
													onclick="document.location='M_Update.do?m_number=${article.m_number}&pageNum=${pageNum}'">수정</button>
												<button type="button" class="btn btn-danger btn-sm"
													onclick="document.location='M_Delete.do?m_number=${article.m_number}&m_title=${article.m_title}&content_mem_id=${article.mem_id}'">삭제</button>
											</c:if>
										</c:if>
									</div>
								</div>
							...
							<c:if test="${!empty mem_id}">
								<tr>
									<form id="writeCommentForm" role="form" class="form-horizontal"
										...
										action="M_CommentWrite.do">
										<input type="hidden" name="m_cnumber" value="${m_cnumber}">
										<input type="hidden" name="mem_id" value="${mem_id}">
										<input type="hidden" name="m_number" value="${article.m_number}">
										<input type="hidden" name="m_cnickname" value="${mem_nickname}">
										<input type="hidden" name="m_cref" value="${m_cref}">
										<input type="hidden" name="m_cre_step" value="${m_cre_step}">
										<input type="hidden" name="m_cre_level" value="${m_cre_level}">
										<td>
											<div class="col-md-1">${mem_nickname}</div>
											<div class="col-md-10">
												<textarea name="m_cbody" rows="5" placeholder="내용을 입력하세요."
									...
									<tr>
										<td>
											<div class="col-md-10" align="left">
												<c:if test="${comment.m_cre_level > 0}">
													<div class="inline-block" align="left">
														<img src="../assets/img/level.gif"
															width="${30*comment.m_cre_level}" height="0"> <font
															width="${20*comment.m_cre_level}"
															style="font-size: 1.5rem;">┗</font>
													</div>
												</c:if>
												<c:if test="${comment.m_cnickname!=' '}">
										${comment.m_cnickname}
									</c:if>
											</div>
											...
											<div class="col-md-2" align="left">
												<c:if test="${comment.m_cnickname!=' '}">
													<font style="font-size: 0.8rem"> <fmt:formatDate
															value="${comment.m_cdate}" timeStyle="medium"
															pattern="yy.MM.dd  HH:mm" />
													</font>
												</c:if>
											...
											<div class="col-md-12" align="left">
												...
												<span>${fn:replace(comment.m_cbody, replaceChar, "<br/>")}</span>
											</div>
											<div class="col-md-12" align="left">
												<c:if test="${comment.m_cnickname!=' '}">
													<c:if test="${comment.m_cre_level > 0}">
													...
													<button type="button" class="btn btn-default btn-xs"
														onclick="re(${comment.m_cnumber})">대댓글</button>
													<c:if
														test="${comment.mem_id==mem_id || mem_id.equals('admin')}">
														<button type="button" class="btn btn-default btn-xs"
															onclick="location.href='M_CommentDelete.do?m_cnumber=${comment.m_cnumber}&m_number=${article.m_number}&m_cref=${comment.m_cref}&m_cre_level=${comment.m_cre_level}&m_cmem_id=${comment.mem_id}'">삭제</button>
													</c:if>
												</c:if>

												<c:if test="${!empty mem_id}">

													<div class="hiddenText" id="a${comment.m_cnumber}">
														<!-- hiddenText -->
														<form id="mrecommentform" role="form"
															class="form-horizontal" method="post"
															name="mrecommentform" action="M_CommentWrite.do">
															<input type="hidden" name="m_cnumber"
																value="${comment.m_cnumber}"> <input
																type="hidden" name="mem_id" value="${mem_id}"> <input
																type="hidden" name="m_number"
																value="${article.m_number}"> <input
																type="hidden" name="m_cnickname" value="${mem_nickname}">
															<input type="hidden" name="m_cref"
																value="${comment.m_cref}"> <input type="hidden"
																name="m_cre_step" value="${comment.m_cre_step}">
															<input type="hidden" name="m_cre_level"
																value="${comment.m_cre_level}">
