...
<body id="page-top" class="bg-dark">
		...
		<section class="page-section bg-dark" id="portfolio">
			...
			<c:if test="${count > 0}">
				<table class="table table-hover">
					...
					<c:forEach var="notice" items="${noticeList}">
						<tr style="background-color: rgba(180, 0, 0, 0.2);color:rgb(255, 255, 255);">
					...
					<c:forEach var="article" items="${articleList}">
						<tr>
							...
							<td width="10%" align="center">${article.m_nickname}</td>
							<td width="15%" align="center"><fmt:formatDate value="${article.m_date}" timeStyle="medium" pattern="yy.MM.dd  HH:mm" /></td>
							<td width="5%" align="center">${article.m_count}</td>
						</tr>
			...
			<div class="col-md-12">
				<c:choose>
				<c:when test="${param.m_ref!=null}">
					<div class="col-md-1"><button type="button" class="btn btn-default btn-sm" onclick="location.href='M_List.do'">모든 글</button></div>
				</c:when>
				<c:otherwise>
			...
			<ul class="pagination">
				...
				<c:choose>
					<c:when test="${param.m_ref==null}">
					<a href="M_List.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"> < </a>
					</c:when>
					<c:otherwise>
					<a href="M_List.do?m_ref=${param.m_ref}&pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"> < </a>
					</c:otherwise>
				</c:choose>
				</li>
				</c:if>
				...
				<c:choose>
					<c:when test="${param.m_ref==null}">
					<a href="M_List.do?pageNum=${i}&search=${search}&searchtext=${searchtext}">
					</c:when>
					<c:otherwise>
					<a href="M_List.do?m_ref=${param.m_ref}&pageNum=${i}&search=${search}&searchtext=${searchtext}">
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					...
					<div class="col-md-offset-2 col-md-8 col-md-offset-2">
						<form action="M_List.do" name="bus">
							<c:if test="${param.m_ref!=null}">
								<input type="hidden" name="m_ref" value="${param.m_ref}">
							</c:if>
							<div class="col-md-3">
							<select class="form-control" name="search">
								<option value="m_all">전체</option>
								<option value="m_title">제목</option>
								<option value="m_body">본문</option>
				...
				<div class="col-md-1"><button type="button" class="btn btn-default btn-sm" onclick="location.href='M_Write.do'">글쓰기</button></div>
			</div>
