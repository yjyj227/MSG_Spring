...
<body id="page-top" class="bg-dark">
		...
		<section class="page-section bg-dark" id="portfolio">
			...
			<c:if test="${pgList.count > 0}">
				<table class="table table-hover">
					...
					<c:forEach var="notice" items="${noticeList}">
						<tr style="background-color: rgba(255, 255, 255, 0.2);color:rgb(255, 255, 255);">
					...
					<c:forEach var="article" items="${articleList}">
						<tr>
							...
							<td width="10%" align="center">${article.z_nickname}</td>
							<td width="15%" align="center"><fmt:formatDate value="${article.z_date}" timeStyle="medium" pattern="yy.MM.dd  HH:mm" /></td>
							<td width="5%" align="center">${article.z_count}</td>
						</tr>
			...
			<div class="col-md-12">
				<c:choose>
				<c:when test="${param.z_ref!=null}">
					<div class="col-md-1"><button type="button" class="btn btn-default btn-sm" onclick="location.href='Z_List.do'">모든 글</button></div>
				</c:when>
				<c:otherwise>
			...
			<ul class="pagination">
				...
				<c:choose>
					<c:when test="${param.z_ref==null}">
					<a href="Z_List.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"> < </a>
					</c:when>
					<c:otherwise>
					<a href="Z_List.do?z_ref=${param.z_ref}&pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"> < </a>
					</c:otherwise>
				</c:choose>
				</li>
				</c:if>
				...
				<c:choose>
					<c:when test="${param.z_ref==null}">
					<a href="Z_List.do?pageNum=${i}&search=${search}&searchtext=${searchtext}">
					</c:when>
					<c:otherwise>
					<a href="Z_List.do?z_ref=${param.z_ref}&pageNum=${i}&search=${search}&searchtext=${searchtext}">
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					...
					<div class="col-md-offset-2 col-md-8 col-md-offset-2">
						<form action="Z_List.do" name="bus">
							<c:if test="${param.z_ref!=null}">
								<input type="hidden" name="z_ref" value="${param.z_ref}">
							</c:if>
							<div class="col-md-3">
							<select class="form-control" name="search">
								<option value="z_all">전체</option>
								<option value="z_title">제목</option>
								<option value="z_body">본문</option>
								<option value="z_nickname">닉네임</option>
							</select>
							</div>
							<div class="col-md-7">
				...
				<div class="col-md-1"><button type="button" class="btn btn-default btn-sm" onclick="location.href='Z_Write.do'">글쓰기</button></div>
			</div>
