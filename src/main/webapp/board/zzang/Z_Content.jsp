					...
					<table class="table">
							...
							<tr align="center">
								...
								<c:choose>
										...
										<c:when test="${article.z_category==22}">
											<font style="color: gray">[암흑 마왕 대추적]&nbsp;&nbsp;&nbsp;</font>
										</c:when>
									...
									<c:choose>
										<c:when test="${article.z_ref==1}">
											<font style="color: gray;">[자유]&nbsp;</font>
										</c:when>
										<c:when test="${article.z_ref==2}">
											<font style="color: gray">[정보]&nbsp;</font>
										</c:when>
									</c:choose> ${article.z_title}</td>
							</tr>
						</thead>
						<tr>
							<td>
								<div class="col-md-1">${article.z_nickname}</div>
								<div class="col-md-7"></div>
								<div class="col-md-2">
									<fmt:formatDate value="${article.z_date}" timeStyle="medium"
										pattern="yy.MM.dd  HH:mm" />
								</div>
								<div class="col-md-1">조회 ${article.z_count}</div>
								<div class="col-md-1">스크랩 ${article.z_scrap}</div>
							</td>
						</tr>
						...
						<tr>
							<td>
								...
								<c:if test="${!empty article.z_filename}">
								<img src="./board/zzang/upload/${article.z_filename}" width="60%">
								<br><br>
								</c:if>
								${fn:replace(article.z_body, replaceChar, "<br/>")}
								</div>
							</td>
						</tr>
						<tr>
								...
								<div class="col-md-12">
									<div class="col-md-2" align="left">
										<button type="button" class="btn btn-default btn-sm"
											onclick="document.location='Z_List.do?z_ref=${article.z_ref}'">목록으로</button>
									</div>
									...
									<div class="col-md-8" align="right">
										<c:if test="${!empty mem_id}">
											<c:if test="${article.mem_id!=mem_id}">
												...
												onclick="document.location='Scrap.do?s_number=${article.z_number}&s_category=${article.z_category}&s_title=${article.z_title}&s_nickname=${article.z_nickname}'">스크랩</button>
											</c:if>

											<c:if
												test="${article.mem_id==mem_id || mem_id.equals('admin')}">
												<button type="button" class="btn btn-warning btn-sm"
													onclick="document.location='Z_Update.do?z_number=${article.z_number}&pageNum=${pageNum}'">수정</button>
												<button type="button" class="btn btn-danger btn-sm"
													onclick="document.location='Z_Delete.do?z_number=${article.z_number}&z_title=${article.z_title}&content_mem_id=${article.mem_id}'">삭제</button>
											</c:if>
										</c:if>
									</div>
								</div>
							...
							<c:if test="${!empty mem_id}">
								<tr>
									<form id="writeCommentForm" role="form" class="form-horizontal"
										method="post" name="zcommentform"
										action="Z_CommentWrite.do">
										<input type="hidden" name="z_cnumber" value="${z_cnumber}">
										<input type="hidden" name="mem_id" value="${mem_id}">
										<input type="hidden" name="z_number" value="${article.z_number}">
										<input type="hidden" name="z_cnickname" value="${mem_nickname}">
										<input type="hidden" name="z_cref" value="${z_cref}">
										<input type="hidden" name="z_cre_step" value="${z_cre_step}">
										<input type="hidden" name="z_cre_level" value="${z_cre_level}">
										<td>
											<div class="col-md-1">${mem_nickname}</div>
											<div class="col-md-10">
												<textarea name="z_cbody" rows="5" placeholder="내용을 입력하세요."
									...
									<tr>
										<td>
											<div class="col-md-10" align="left">
												<c:if test="${comment.z_cre_level > 0}">
													<div class="inline-block" align="left">
														<img src="../assets/img/level.gif"
															width="${30*comment.z_cre_level}" height="0"> <font
															width="${20*comment.z_cre_level}"
															style="font-size: 1.5rem;">┗</font>
													</div>
												</c:if>
												<c:if test="${comment.z_cnickname!=' '}">
										${comment.z_cnickname}
									</c:if>
											</div>
											...
											<div class="col-md-2" align="left">
												<c:if test="${comment.z_cnickname!=' '}">
													<font style="font-size: 0.8rem"> <fmt:formatDate
															value="${comment.z_cdate}" timeStyle="medium"
															pattern="yy.MM.dd  HH:mm" />
													</font>
												</c:if>
											...
											<div class="col-md-12" align="left">
												...
												<span>${fn:replace(comment.z_cbody, replaceChar, "<br/>")}</span>
											</div>
											<div class="col-md-12" align="left">
												<c:if test="${comment.z_cnickname!=' '}">
													<c:if test="${comment.z_cre_level > 0}">
													...
													<button type="button" class="btn btn-default btn-xs"
														onclick="re(${comment.z_cnumber})">대댓글</button>
													<c:if
														test="${comment.mem_id==mem_id || mem_id.equals('admin')}">
														<button type="button" class="btn btn-default btn-xs"
															onclick="location.href='Z_CommentDelete.do?z_cnumber=${comment.z_cnumber}&z_number=${article.z_number}&z_cref=${comment.z_cref}&z_cre_level=${comment.z_cre_level}&z_cmem_id=${comment.mem_id}'">삭제</button>
													</c:if>
												</c:if>

												<c:if test="${!empty mem_id}">

													<div class="hiddenText" id="a${comment.z_cnumber}">
														<!-- hiddenText -->
														<form id="zrecommentform" role="form"
															class="form-horizontal" method="post"
															name="zrecommentform" action="Z_CommentWrite.do">
															<input type="hidden" name="z_cnumber"
																value="${comment.z_cnumber}"> <input
																type="hidden" name="mem_id" value="${mem_id}"> <input
																type="hidden" name="z_number"
																value="${article.z_number}"> <input
																type="hidden" name="z_cnickname" value="${mem_nickname}">
															<input type="hidden" name="z_cref"
																value="${comment.z_cref}"> <input type="hidden"
																name="z_cre_step" value="${comment.z_cre_step}">
															<input type="hidden" name="z_cre_level"
																value="${comment.z_cre_level}">
