			...
			<div class="row">
				<div class="col-md-offset-1 col-md-10 col-md-offset-1">
					...
					<table class="table">
						<thead>
							<tr align="center">
								...
								<c:choose>
									...
									<c:choose>
									<c:when test="${article.g_ref==1}">
										<font style="color: gray;">[자유]&nbsp;</font>
									</c:when>
									<c:when test="${article.g_ref==2}">
						...
						<tr>
							<td>
								<div class="col-md-1">${article.g_nickname}</div>
								<div class="col-md-7"></div>
								<div class="col-md-2">
									<fmt:formatDate value="${article.g_date}" timeStyle="medium"
										pattern="yy.MM.dd  HH:mm" />
								</div>
								<div class="col-md-1">조회 ${article.g_count}</div>
								<div class="col-md-1">스크랩 ${article.g_scrap}</div>
							</td>
						</tr>
						...
						<tr>
							<td>
								...
								<c:if test="${!empty article.g_filename}">
								<img src="./board/ghibli/upload/${article.g_filename}" width="60%">
								<br><br>
								</c:if>
								${fn:replace(article.g_body, replaceChar, "<br/>")}
								</div>
							</td>
						</tr>
						<tr>
								...
								<div class="col-md-12">
									<div class="col-md-2" align="left">
										<button type="button" class="btn btn-default btn-sm"
											onclick="document.location='G_List.do?g_ref=${article.g_ref}'">목록으로</button>
									</div>
									...
									<div class="col-md-8" align="right">
										<c:if test="${!empty mem_id}">
											<c:if test="${article.mem_id!=mem_id}">
												...
												onclick="document.location='Scrap.do?s_number=${article.g_number}&s_category=${article.g_category}&s_title=${article.g_title}&s_nickname=${article.g_nickname}'">스크랩</button>
											</c:if>

											<c:if
												test="${article.mem_id==mem_id || mem_id.equals('admin')}">
												<button type="button" class="btn btn-warning btn-sm"
													onclick="document.location='G_Update.do?g_number=${article.g_number}&pageNum=${pageNum}'">수정</button>
												<button type="button" class="btn btn-danger btn-sm"
													onclick="document.location='G_Delete.do?g_number=${article.g_number}&g_title=${article.g_title}&content_mem_id=${article.mem_id}'">삭제</button>
											</c:if>
										</c:if>
									</div>
								</div>
							...
							<c:if test="${!empty mem_id}">
								<tr>
									<form id="writeCommentForm" role="form" class="form-horizontal"
										method="post" name="gcommentform"
										action="G_CommentWrite.do">
										<input type="hidden" name="g_cnumber" value="${g_cnumber}">
										<input type="hidden" name="mem_id" value="${mem_id}">
										<input type="hidden" name="g_number" value="${article.g_number}">
										<input type="hidden" name="g_cnickname" value="${mem_nickname}">
										<input type="hidden" name="g_cref" value="${g_cref}">
										<input type="hidden" name="g_cre_step" value="${g_cre_step}">
										<input type="hidden" name="g_cre_level" value="${g_cre_level}">
										<td>
											<div class="col-md-1">${mem_nickname}</div>
											<div class="col-md-10">
												<textarea name="g_cbody" rows="5" placeholder="내용을 입력하세요."
									...
									<tr>
										<td>
											<div class="col-md-10" align="left">
												<c:if test="${comment.g_cre_level > 0}">
													<div class="inline-block" align="left">
														<img src="../assets/img/level.gif"
															width="${30*comment.g_cre_level}" height="0"> <font
															width="${20*comment.g_cre_level}"
															style="font-size: 1.5rem;">┗</font>
													</div>
												</c:if>
												<c:if test="${comment.g_cnickname!=' '}">
										${comment.g_cnickname}
									</c:if>
											</div>
											...
											<div class="col-md-2" align="left">
												<c:if test="${comment.g_cnickname!=' '}">
													<font style="font-size: 0.8rem"> <fmt:formatDate
															value="${comment.g_cdate}" timeStyle="medium"
															pattern="yy.MM.dd  HH:mm" />
													</font>
												</c:if>
											...
											<div class="col-md-12" align="left">
												...
												<span>${fn:replace(comment.g_cbody, replaceChar, "<br/>")}</span>
											</div>
											<div class="col-md-12" align="left">
												<c:if test="${comment.g_cnickname!=' '}">
													<c:if test="${comment.g_cre_level > 0}">
													...
													<button type="button" class="btn btn-default btn-xs"
														onclick="re(${comment.g_cnumber})">대댓글</button>
													<c:if
														test="${comment.mem_id==mem_id || mem_id.equals('admin')}">
														<button type="button" class="btn btn-default btn-xs"
															onclick="location.href='G_CommentDelete.do?g_cnumber=${comment.g_cnumber}&g_number=${article.g_number}&g_cref=${comment.g_cref}&g_cre_level=${comment.g_cre_level}&g_cmem_id=${comment.mem_id}'">삭제</button>
													</c:if>
												</c:if>

												<c:if test="${!empty mem_id}">

													<div class="hiddenText" id="a${comment.g_cnumber}">
														<!-- hiddenText -->
														<form id="grecommentform" role="form"
															class="form-horizontal" method="post"
															name="grecommentform" action="G_CommentWrite.do">
															<input type="hidden" name="g_cnumber"
																value="${comment.g_cnumber}"> <input
																type="hidden" name="mem_id" value="${mem_id}"> <input
																type="hidden" name="g_number"
																value="${article.g_number}"> <input
																type="hidden" name="g_cnickname" value="${mem_nickname}">
															<input type="hidden" name="g_cref"
																value="${comment.g_cref}"> <input type="hidden"
																name="g_cre_step" value="${comment.g_cre_step}">
															<input type="hidden" name="g_cre_level"
																value="${comment.g_cre_level}">
