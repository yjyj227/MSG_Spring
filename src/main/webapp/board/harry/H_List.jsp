		...
		<section class="page-section bg-dark" id="portfolio">
			...
			<c:if test="${count > 0}">
				<table class="table table-hover">
					...
					<c:set var="numberN" value="${pgList2.number}" />
					<c:forEach var="notice" items="${noticeList}">
						<tr style="background-color: rgba(104, 145, 91, 0.2);color:rgb(255, 255, 255);">
							...
							<a href="N_Content.do?notice_number=${notice.notice_number}&pageNum=${pgListN.currentPage}">${notice.not_title}</a>
							</td>
			...
			<ul class="pagination">
				...
				<c:if test="${pgList.startPage > pgList.blockSize}">
					<li>
						<c:choose>
							<c:when test="${param.h_ref==null}">
							<a href="H_List.do?pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"> < </a>
							</c:when>
							<c:otherwise>
							<a href="H_List.do?h_ref=${param.h_ref}&pageNum=${pgList.startPage-pgList.blockSize}&search=${search}&searchtext=${searchtext}"> < </a>
							</c:otherwise>
						</c:choose>
					</li>
				</c:if>
				...
				<c:choose>
					...
					<c:otherwise>
					<a href="H_List.do?h_ref=${param.h_ref}&pageNum=${i}&search=${search}&searchtext=${searchtext}">
					</c:otherwise>
				</c:choose>
				...
				<c:if test="${pgList.endPage < pgList.pageCount}">
					...
					<div class="col-md-offset-2 col-md-8 col-md-offset-2">
						<form action="H_List.do" name="bus">
							<c:if test="${param.h_ref!=null}">
								<input type="hidden" name="h_ref" value="${param.h_ref}">
							</c:if>
							<div class="col-md-3">
							<select class="form-control" name="search">
								<option value="h_all">전체</option>
								<option value="h_title">제목</option>
								<option value="h_body">본문</option>
				...
				<c:if test="${!empty mem_id}">
				<div class="col-md-1"><button type="button" class="btn btn-default btn-sm" onclick="location.href='H_Write.do'">글쓰기</button></div>
				</c:if>
			</div>
