...
<body id="page-top" class="bg-dark">
		...
		<section class="page-section bg-dark" id="portfolio">
			...
			<table class="table">
					<thead>
						<tr align="center">
							<td>${article.not_title}</td>
						</tr>
					</thead>
					<tr>
						<td>
							<div class="col-md-1">${article.admin_id}</div>
							<div class="col-md-8"></div>
							<div class="col-md-2"><fmt:formatDate value="${article.not_date}" timeStyle="medium" pattern="yy.MM.dd  hh:mm" /></div>
							<div class="col-md-1">조회 ${article.not_count}</div>
						</td>
					</tr>
					...
					<tr>
						...
						<c:if test="${!empty article.not_filename}">
						<img src="./board/notice/upload/${article.not_filename}" width="60%">
						<br><br>
						</c:if>
						${fn:replace(article.not_body, replaceChar, "<br/>")}
						</div>
						</td>
					</tr>
					<tr>
						...
						<button type="button" class="btn btn-default btn-sm" onclick="document.location='N_List.do'">목록으로</button>
						</div>
						<div class="col-md-2"></div>
						<div class="col-md-8" align="right">
					<c:if test="${!empty mem_id}">
						<c:if test="${mem_id.equals('admin')}">
							<button type="button" class="btn btn-warning btn-sm" onclick="document.location='N_Update.do?notice_number=${article.notice_number}'">수정</button>
							<button type="button" class="btn btn-danger btn-sm" onclick="document.location='N_Delete.do?notice_number=${article.notice_number}'">삭제</button>
						</c:if>
					</c:if>
